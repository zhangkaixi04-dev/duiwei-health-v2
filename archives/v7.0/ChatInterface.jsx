...
const WeeklyReportCard = ({ report }) => {
    ...
    useEffect(() => {
        const updateFromStorage = () => {
            ...
            const hasSleep = !!dailyLogs.sleep;
            const hasExercise = !!dailyLogs.exercise;
            const hasDiet = !!dailyLogs.diet && dailyLogs.diet.length > 0;
            const hasPoop = !!dailyLogs.poop;
    ...
    <div className="bg-gradient-to-br from-[#FAF9F6] to-[#EDF5F0] p-5 rounded-2xl border border-gray-100 shadow-lg space-y-5 animate-fade-in-up mx-2 mb-4">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-lg font-bold text-brand font-serif">健康洞察</h3>
          ...
          <div className="space-y-4">
              <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                  <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-bold text-text-main">今日状态总览</h4>
                  </div>
                  <div className="space-y-2">
                      <div className="text-sm font-medium text-text-main">{healthStatusLabel}</div>
                      <div className="text-xs text-text-muted">
                          已记录：
                          {dailyLog.records.diet ? ' 饮食 ✓' : ' 饮食 -'}
                          {dailyLog.records.sleep ? ' 睡眠 ✓' : ' 睡眠 -'}
                          {dailyLog.records.exercise ? ' 运动 ✓' : ' 运动 -'}
                          {dailyLog.records.poop ? ' 排便 ✓' : ' 排便 -'}
                      </div>
                  </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                      ...
                      <div>
                          <div className="text-xs text-text-muted">睡眠记录</div>
                          <div className="text-sm font-bold text-text-main">{dailyLog.records.sleep ? '✓ 已记录' : '- 未记录'}</div>
                      </div>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                      ...
                      <div>
                          <div className="text-xs text-text-muted">运动记录</div>
                          <div className="text-sm font-bold text-text-main">{dailyLog.records.exercise ? '✓ 已记录' : '- 未记录'}</div>
                      </div>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                      ...
                      <div>
                          <div className="text-xs text-text-muted">饮食记录</div>
                          <div className="text-sm font-bold text-text-main">{dailyLog.records.diet ? `✓ ${dailyLog.food_variety}种` : '- 未记录'}</div>
                      </div>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                      ...
                      <div>
                          <div className="text-xs text-text-muted">排便记录</div>
                          <div className="text-sm font-bold text-text-main">{dailyLog.records.poop ? '✓ 已记录' : '- 未记录'}</div>
                      </div>
                  </div>
              </div>
          </div>
